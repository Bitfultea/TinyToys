...
void MemoryManager::Memcpy(void *dst_ptr, const Device &dst_device, const void *src_ptr,
                           const Device &src_device, size_t num_bytes) {
    ...
    // Not supporting other combinations at the moment, e.g. SYCL->CUDA.
        ...
        utility::LogError("Unsupported device type from {} to {}.", src_device.ToString(),
                          dst_device.ToString());
    }
...
void MemoryManager::MemcpyFromHost(void *dst_ptr, const Device &dst_device,
                                   const void *host_ptr, size_t num_bytes) {
    // Currently default host is CPU:0
    Memcpy(dst_ptr, dst_device, host_ptr, Device("CPU:0"), num_bytes);
}

void MemoryManager::MemcpyToHost(void *host_ptr, const void *src_ptr,
                                 const Device &src_device, size_t num_bytes) {
    // Currently default host is CPU:0
...
MemoryManager::GetMemoryManagerDevice(const Device &device) {
    static std::unordered_map<Device::DeviceType, std::shared_ptr<MemoryManagerDevice>,
                              utility::hash_enum_class>
        map_device_type_to_memory_manager = {
            {Device::DeviceType::CPU, std::make_shared<MemoryManagerCPU>()},
#ifdef BUILD_CUDA_MODULE
#ifdef ENABLE_CACHED_CUDA_MANAGER
            {Device::DeviceType::CUDA, std::make_shared<MemoryManagerCached>(
                                           std::make_shared<MemoryManagerCUDA>())},
#else
            {Device::DeviceType::CUDA, std::make_shared<MemoryManagerCUDA>()},
#endif
#endif
#ifdef BUILD_SYCL_MODULE
            {Device::DeviceType::SYCL, std::make_shared<MemoryManagerSYCL>()},
#endif
        };

    if (map_device_type_to_memory_manager.find(device.GetType())
        == map_device_type_to_memory_manager.end()) {
        utility::LogError("Unsupported device \"{}\". Set BUILD_CUDA_MODULE=ON to "
