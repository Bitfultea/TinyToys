...
static Tensor ConcatenateImpl(const std::vector<Tensor> &tensors, const int64_t axis) {
    ...
    for (int i = 1; i < num_tensors; ++i) {
        ...
        if (tensors[i].NumDims() != num_dims) {
            utility::LogError("All the input tensors must have same number of "
        ...
        for (int64_t j = 0; j < num_dims; ++j) {
            if (j != axis_d && combined_shape[j] != tensors[i].GetShape(j)) {
                utility::LogError("All the input tensor dimensions, other than dimension "
    ...
    for (int i = 0; i < num_tensors; ++i) {
        ...
        local_tks.push_back(
            TensorKey::Slice(cumulated_length, cumulated_length + local_length, 1));
...
Tensor Concatenate(const std::vector<Tensor> &tensors,
    ...
    if (num_tensors < 1) { utility::LogError("Expected at least 1 tensor, but got 0."); }
    if (num_tensors == 1) {
    ...
    if (!axis.has_value()) {
        ...
        if (tensors[0].NumDims() == 0) {
            utility::LogError("Zero-dimensional tensor can only be concatenated along "
                              "axis = null, but got {}.",
                              axis.value());
        }
...
Tensor Append(const Tensor &self, const Tensor &other,
              const utility::optional<int64_t> &axis) {
...
Tensor Maximum(const Tensor &input, const Tensor &other) {
    ...
    Tensor dst_tensor(shape_util::BroadcastedShape(input.GetShape(), other.GetShape()),
