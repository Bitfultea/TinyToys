...
namespace open3d { namespace core { namespace sycl {
...
int SYCLDemo() {
    ...
    for (size_t i = 0; i < buffer.size(); ++i) {
        if (host_accessor[i] != i) {
            utility::LogInfo("Mismatch found at index {}: expected {}, got {}.", i, i,
                             host_accessor[i]);
            mismatch_found = true;
        }
    }
...
static std::string GetDeviceTypeName(const sy::device &device) {
    ...
    switch (device_type) {
    case sy::info::device_type::cpu: return "cpu";
    case sy::info::device_type::gpu: return "gpu";
    case sy::info::device_type::host: return "host";
    case sy::info::device_type::accelerator: return "acc";
...
void PrintSYCLDevices(bool print_all) {
    ...
    if (print_all) {
        ...
        const std::vector<sy::platform> &platforms = sy::platform::get_platforms();
        ...
        try {
            const sy::device &device = sy::device(sy::gpu_selector());
            utility::LogInfo("- Device(\"SYCL:0\"): {}", SYCLDeviceToString(device));
        } catch (const sy::exception &e) {
...
#else
    utility::LogInfo("PrintSYCLDevices is not compiled with BUILD_SYCL_MODULE=ON.");
#endif
}
