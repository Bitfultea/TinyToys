...
#define LogError(...)                                                                    \
    Logger::LogError_(__FILE__, __LINE__, static_cast<const char *>(OPEN3D_FUNCTION),    \
                      __VA_ARGS__)
...
#define LogWarning(...)                                                                  \
    Logger::LogWarning_(__FILE__, __LINE__, static_cast<const char *>(OPEN3D_FUNCTION),  \
                        __VA_ARGS__)
...
#define LogInfo(...)                                                                     \
    Logger::LogInfo_(__FILE__, __LINE__, static_cast<const char *>(OPEN3D_FUNCTION),     \
                     __VA_ARGS__)
...
#define LogDebug(...)                                                                    \
    Logger::LogDebug_(__FILE__, __LINE__, static_cast<const char *>(OPEN3D_FUNCTION),    \
                      __VA_ARGS__)

namespace open3d { namespace utility {
...
public:
    ...
    static void LogError_ [[noreturn]] (const char *file, int line, const char *function,
                                        const char *format, Args &&...args) {
        if (sizeof...(Args) > 0) {
            Logger::GetInstance().VError(
                file, line, function, FormatArgs(format, fmt::make_format_args(args...)));
        } else {
            Logger::GetInstance().VError(file, line, function, std::string(format));
        }
    }
    template <typename... Args>
    static void LogWarning_(const char *file, int line, const char *function,
        ...
        if (Logger::GetInstance().GetVerbosityLevel() >= VerbosityLevel::Warning) {
            if (sizeof...(Args) > 0) {
                ...
                Logger::GetInstance().VWarning(file, line, function, std::string(format));
            }
        }
    }
    template <typename... Args>
    static void LogInfo_(const char *file, int line, const char *function,
        ...
        if (Logger::GetInstance().GetVerbosityLevel() >= VerbosityLevel::Info) {
            if (sizeof...(Args) > 0) {
                ...
                Logger::GetInstance().VInfo(file, line, function, std::string(format));
            }
        }
    }
    template <typename... Args>
    static void LogDebug_(const char *file, int line, const char *function,
        ...
        if (Logger::GetInstance().GetVerbosityLevel() >= VerbosityLevel::Debug) {
            if (sizeof...(Args) > 0) {
                ...
                Logger::GetInstance().VDebug(file, line, function, std::string(format));
            }
        }
    }
...
public:
    ...
    void Exit() {
        Logger::GetInstance().SetVerbosityLevel(level_backup_);
    }
