...
public:
    ...
    Scalar(uint64_t v) {
        ...
        if (v <= static_cast<uint64_t>(std::numeric_limits<int64_t>::max())) {
            ...
            value_.i =
                -static_cast<int64_t>(std::numeric_limits<uint64_t>::max() - v) - 1;
        }
    }
    Scalar(bool v) {
    ...
    bool IsDouble() const {
        return scalar_type_ == ScalarType::Double;
    }
    bool IsInt64() const {
        return scalar_type_ == ScalarType::Int64;
    }
    bool IsBool() const {
    ...
    double GetDouble() const {
        if (!IsDouble()) { utility::LogError("Scalar is not a ScalarType:Double type."); }
        return value_.d;
    }
    /// Returns int64 value from Scalar. Only works when scalar_type_ is
    /// ScalarType::Int64.
    int64_t GetInt64() const {
        if (!IsInt64()) { utility::LogError("Scalar is not a ScalarType:Int64 type."); }
        return value_.i;
    }
    /// Returns bool value from Scalar. Only works when scalar_type_ is
    /// ScalarType::Bool.
    bool GetBool() const {
        if (!IsBool()) { utility::LogError("Scalar is not a ScalarType:Bool type."); }
        return value_.b;
    }
    ...
    void AssertSameScalarType(Scalar other, const std::string &error_msg) const {
        if (scalar_type_ != other.scalar_type_) {
            if (error_msg.empty()) {
                utility::LogError("Scalar mode {} are not the same as {}.", ToString(),
                                  other.ToString());
            } else {
                utility::LogError("Scalar mode {} are not the same as {}: {}", ToString(),
                                  other.ToString(), error_msg);
            }
        }
    }
