...
public:
        ...
        if (free_block != nullptr) {
            ...
            if (remaining_size == 0) {
                ...
                auto remaining_block = std::make_shared<VirtualBlock>(
                    static_cast<char *>(free_block->ptr_) + byte_size, remaining_size,
                    free_block->r_block_);
    ...
    void Free(void *ptr) {
        ...
        if (ptr_it == allocated_virtual_blocks_.end()) {
            ...
            utility::LogError("Block of {} should have been recorded.", fmt::ptr(ptr));
        }
        ...
        if (v_block_it == v_block_set.end()) {
            utility::LogError("Virtual block ({} @ {} bytes) not recorded in real block "
                              "{} @ {} bytes.",
        ...
        if (v_block_it != v_block_set.begin()) {
            ...
            if (free_virtual_blocks_.find(v_block_prev) != free_virtual_blocks_.end()) {
                ...
                merged_v_block = std::make_shared<VirtualBlock>(
                    v_block_prev->ptr_,
                    v_block_prev->byte_size_ + merged_v_block->byte_size_, r_block);
        ...
        if (v_block_it_next != v_block_set.end()) {
            ...
            if (free_virtual_blocks_.find(v_block_next) != free_virtual_blocks_.end()) {
                ...
                merged_v_block = std::make_shared<VirtualBlock>(
                    merged_v_block->ptr_,
                    merged_v_block->byte_size_ + v_block_next->byte_size_, r_block);
    ...
    void Acquire(void *ptr, size_t byte_size,
    ...
    Release(size_t byte_size) {
        std::lock_guard<std::recursive_mutex> lock(mutex_);

        // Filter releasable blocks.
        std::set<std::shared_ptr<RealBlock>, SizeOrder<RealBlock>> releasable_real_blocks;
        std::copy_if(
            real_blocks_.begin(), real_blocks_.end(),
            std::inserter(releasable_real_blocks, releasable_real_blocks.begin()),
        ...
        while (!releasable_real_blocks.empty() && released_size < byte_size) {
            ...
            auto query_size = std::make_shared<RealBlock>(nullptr, remaining_size);
            auto it = releasable_real_blocks.lower_bound(query_size);
            if (it == releasable_real_blocks.end()) { --it; }
            auto r_block = *it;
...
private:
    ...
    std::shared_ptr<VirtualBlock> ExtractFreeBlock(size_t byte_size) {
        ...
        size_t max_byte_size =
            static_cast<size_t>(kMaxFragmentation * static_cast<double>(byte_size));

        // Consider blocks with size in range
        // [byte_size, max_byte_size].
        auto query_size = std::make_shared<VirtualBlock>(nullptr, byte_size,
                                                         std::weak_ptr<RealBlock>());
        auto it = free_virtual_blocks_.lower_bound(query_size);
        while (it != free_virtual_blocks_.end() && (*it)->byte_size_ <= max_byte_size) {
    ...
    std::unordered_map<void *, std::shared_ptr<VirtualBlock>> allocated_virtual_blocks_;
    std::set<std::shared_ptr<VirtualBlock>, SizeOrder<VirtualBlock>> free_virtual_blocks_;
...
public:
    ...
    ~Cacher() {
        for (const auto &cache_pair : device_caches_) {
            ...
            if (!cache.Empty()) {
                utility::LogError("{} leaking memory blocks on {}", cache.Size(),
                                  device.ToString());
            }
        }
    }
    ...
    void *Malloc(size_t byte_size, const Device &device,
        ...
        if (ptr == nullptr) {
            auto old_ptrs = device_caches_.at(device).Release(internal_byte_size);
    ...
    void Clear() {
        ...
        for (const auto &device : devices) { Clear(device); }
    }
...
private:
    ...
    void Init(const Device &device) {
        ...
        device_caches_.emplace(std::piecewise_construct, std::forward_as_tuple(device),
...
MemoryManagerCached::MemoryManagerCached(
    const std::shared_ptr<MemoryManagerDevice> &device_mm) : device_mm_(device_mm) {
    if (std::dynamic_pointer_cast<MemoryManagerCached>(device_mm_) != nullptr) {
        utility::LogError("An instance of type MemoryManagerCached as the underlying "
...
void MemoryManagerCached::Memcpy(void *dst_ptr, const Device &dst_device,
                                 const void *src_ptr, const Device &src_device,
                                 size_t num_bytes) {
...
void MemoryManagerCached::ReleaseCache() {
    Cacher::GetInstance().Clear();
}
