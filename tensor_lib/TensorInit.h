...
namespace open3d { namespace core { namespace tensor_init {
    ...
    static constexpr size_t value(const L &list) {
        ...
        for (const auto &value : list) {
            if (dim != InitializerShapeImpl<D - 1>::value(value)) {
                utility::LogError("Input contains ragged nested sequences"
...
SizeVector InitializerShape(const L &list, std::index_sequence<D...>) {
    return SizeVector{static_cast<int64_t>(InitializerShapeImpl<D>::value(list))...};
}

template <typename L>
SizeVector InferShape(const L &list) {
    SizeVector shape =
        InitializerShape<L>(list, std::make_index_sequence<InitializerDim<L>::value>());
    // Handle 0-dimensional inputs.
    size_t last_dim = 0;
    while (shape.size() > (last_dim + 1) && shape[last_dim] != 0) { last_dim++; }
...
void NestedCopy(T &&iter, const std::initializer_list<L> &list) {
    for (const auto &value : list) { NestedCopy(std::forward<T>(iter), value); }
}

template <typename T, size_t D>
std::vector<T> ToFlatVector(const SizeVector &shape,
