...
MemoryManagerStatistic::~MemoryManagerStatistic() {
    if (print_at_program_end_) {
        ...
        if (HasLeaks()) { std::exit(EXIT_FAILURE); }
    }
}

void MemoryManagerStatistic::SetPrintLevel(PrintLevel level) {
    level_ = level;
}
...
void MemoryManagerStatistic::Print() const {
    if (level_ == PrintLevel::None) { return; }

    if (level_ == PrintLevel::Unbalanced && !HasLeaks()) { return; }
    ...
    for (const auto &value_pair : statistics_) {
        ...
        if (!statistics.IsBalanced()) {
            ...
            utility::LogInfo("{}: {} {}", device.ToString(), statistics.count_malloc_,
                             statistics.count_free_);
        }
    }
    utility::LogInfo("---------------------------------------------");
...
bool MemoryManagerStatistic::HasLeaks() const {
    return std::any_of(
        statistics_.begin(), statistics_.end(),
        [](const auto &value_pair) -> bool { return !value_pair.second.IsBalanced(); });
}

void MemoryManagerStatistic::CountMalloc(void *ptr, size_t byte_size,
                                         const Device &device) {
    std::lock_guard<std::mutex> lock(statistics_mutex_);

    // Filter nullptr. Empty allocations are not tracked.
    if (ptr == nullptr && byte_size == 0) { return; }
    ...
    if (it.second) {
        ...
        if (print_at_malloc_free_) {
            utility::LogInfo("[Malloc] {}: {} @ {} bytes",
                             fmt::sprintf("%6s", device.ToString()), fmt::ptr(ptr),
                             byte_size);
        }
    } else {
        utility::LogError("{} @ {} bytes on {} is still active and was not freed before",
...
void MemoryManagerStatistic::CountFree(void *ptr, const Device &device) {
    ...
    if (num_to_erase == 1) {
        ...
        utility::LogError("Invalid number of erased allocations {} for {} on {}",
