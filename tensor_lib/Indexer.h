...
#pragma unroll
#endif
        for (int arg = 0; arg < NARGS; arg++) { offsets[arg] = 0; }
...
struct TensorRef {
    ...
    TensorRef(const Tensor &t) {
        if (t.NumDims() > MAX_DIMS) {
            utility::LogError("Tenor has too many dimensions {} > {}.", t.NumDims(),
                              MAX_DIMS);
        }
    ...
    void Permute(const SizeVector &dims) {
        ...
        if (static_cast<int64_t>(dims.size()) != ndims_) {
            utility::LogError("Number of dimensions mismatch {} != {}.", dims.size(),
                              ndims_);
        }
        std::vector<bool> seen_dims(ndims_, false);
        for (const int64_t &dim : dims) { seen_dims[dim] = true; }
...
public:
    ...
    OPEN3D_HOST_DEVICE int64_t NumWorkloads() const {
        ...
        for (int64_t i = 0; i < ndims_; ++i) { num_workloads *= input_.shape_[i]; }
        return num_workloads;
    }

    OPEN3D_HOST_DEVICE void *GetPtr(int64_t workload_idx) const {
        if (workload_idx < 0 || workload_idx >= NumWorkloads()) { return nullptr; }
        int64_t offset = 0;
        workload_idx = workload_idx * input_.dtype_byte_size_;
        for (int64_t i = 0; i < ndims_; ++i) {
            offset += workload_idx / input_.byte_strides_[i] * input_.byte_strides_[i];
            workload_idx = workload_idx % input_.byte_strides_[i];
        }
        return static_cast<void *>(static_cast<char *>(input_.data_ptr_) + offset);
    }
...
public:
    ...
    /// all outputs.
    Indexer(const std::vector<Tensor> &input_tensors, const Tensor &output_tensor,
    ...
    TensorRef &GetInput(int64_t i) {
        if (i >= num_inputs_ || i < 0) {
            utility::LogError("0 <= i < {} required, however, i = {}.", num_inputs_, i);
        }
        return inputs_[i];
    }
    ...
    TensorRef &GetOutput(int64_t i) {
        if (i >= num_outputs_ || i < 0) {
            utility::LogError("0 <= i < {} required, however, i = {}.", num_outputs_, i);
        }
        return outputs_[i];
    }
    ...
    TensorRef &GetOutput() {
        if (num_outputs_ > 1) {
            utility::LogError("num_outputs_ == {} > 0, use GetOutput(i)", num_outputs_);
        }
        return GetOutput(0);
    }
    ...
    OPEN3D_HOST_DEVICE char *GetInputPtr(int64_t input_idx, int64_t workload_idx) const {
        if (input_idx < 0 || input_idx >= num_inputs_) { return nullptr; }
        return GetWorkloadDataPtr(inputs_[input_idx], inputs_contiguous_[input_idx],
                                  workload_idx);
    }
    ...
    OPEN3D_HOST_DEVICE T *GetInputPtr(int64_t input_idx, int64_t workload_idx) const {
        if (input_idx < 0 || input_idx >= num_inputs_) { return nullptr; }
        return GetWorkloadDataPtr<T>(inputs_[input_idx], inputs_contiguous_[input_idx],
                                     workload_idx);
    }
    ...
    OPEN3D_HOST_DEVICE T *GetOutputPtr(int64_t workload_idx) const {
        return GetWorkloadDataPtr<T>(outputs_[0], outputs_contiguous_[0], workload_idx);
    }
    ...
    OPEN3D_HOST_DEVICE char *GetOutputPtr(int64_t output_idx,
        ...
        return GetWorkloadDataPtr(outputs_[output_idx], outputs_contiguous_[output_idx],
                                  workload_idx);
    }
    ...
    OPEN3D_HOST_DEVICE T *GetOutputPtr(int64_t output_idx, int64_t workload_idx) const {
        return GetWorkloadDataPtr<T>(outputs_[output_idx],
                                     outputs_contiguous_[output_idx], workload_idx);
    }
...
protected:
    ...
    static void BroadcastRestride(TensorRef &src, int64_t dst_ndims,
    ...
    static void ReductionRestride(TensorRef &dst, int64_t src_ndims,
    ...
    /// together.
    OPEN3D_HOST_DEVICE char *GetWorkloadDataPtr(const TensorRef &tr, bool tr_contiguous,
                                                int64_t workload_idx) const {
        // For 0-sized input reduction op, the output Tensor
        // workload_idx == 1 > NumWorkloads() == 0.
        if (workload_idx < 0) { return nullptr; }
        if (tr_contiguous) {
            return static_cast<char *>(tr.data_ptr_) + workload_idx * tr.dtype_byte_size_;
        } else {
            int64_t offset = 0;
            for (int64_t i = 0; i < ndims_; ++i) {
                offset += workload_idx / primary_strides_[i] * tr.byte_strides_[i];
    ...
    /// not check this constraint for performance reasons.
    template <typename T>
    OPEN3D_HOST_DEVICE T *GetWorkloadDataPtr(const TensorRef &tr, bool tr_contiguous,
                                             int64_t workload_idx) const {
        // For 0-sized input reduction op, the output Tensor
        // workload_idx == 1 > NumWorkloads() == 0.
        if (workload_idx < 0) { return nullptr; }
