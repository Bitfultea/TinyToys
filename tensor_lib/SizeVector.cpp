...
DynamicSizeVector::DynamicSizeVector(const DynamicSizeVector &other) : super_t(other) {
}
...
SizeVector::SizeVector(int64_t n, int64_t initial_value) : super_t(n, initial_value) {
}
...
bool SizeVector::IsCompatible(const DynamicSizeVector &dsv) const {
    if (size() != dsv.size()) { return false; }
    for (size_t i = 0; i < size(); ++i) {
        if (dsv[i].has_value() && dsv[i].value() != this->operator[](i)) { return false; }
    }
    return true;
}
