...
static void AssertIsResizable(const TensorList &tensorlist,
    ...
    if (!tensorlist.IsResizable()) {
        utility::LogError("TensorList::{}: TensorList is not resizable. Typically this "
...
TensorList TensorList::FromTensor(const Tensor &tensor, bool inplace) {
    ...
    SizeVector element_shape = SizeVector(std::next(shape.begin()), shape.end());
    int64_t size = shape[0];

    if (inplace) {
        if (!tensor.IsContiguous()) {
            utility::LogError("Tensor must be contiguous for inplace tensorlist "
                              "construction.");
        }
...
void TensorList::Extend(const TensorList &other) {
    ...
    if (element_shape_ != other.GetElementShape()) {
        utility::LogError("TensorList shapes {} and {} are inconsistent.", element_shape_,
                          other.GetElementShape());
    }
...
void TensorList::ResizeWithExpand(int64_t new_size) {
    ...
    if (new_reserved_size <= reserved_size_) {
        ...
        Tensor new_internal_tensor(
            shape_util::Concat({new_reserved_size}, element_shape_), GetDtype(),
            GetDevice());
        new_internal_tensor.Slice(0, 0, size_) = internal_tensor_.Slice(0, 0, size_);
...
int64_t TensorList::ComputeReserveSize(int64_t n) {
    if (n < 0) { utility::LogError("Negative tensorlist size {} is not supported.", n); }
...
std::string TensorList::ToString() const {
    return fmt::format("TensorList[size: {}, element_shape: {}, dtype: {}, device: {}]",
