...
std::string JoinStrings(const std::vector<std::string> &strs,
    ...
    for (size_t i = 0; i < strs.size(); ++i) {
        oss << strs[i];
        if (i != strs.size() - 1) { oss << delimiter; }
    }
    return oss.str();
}
...
size_t WordLength(const std::string &doc, size_t start_pos,
                  const std::string &valid_chars) {
    std::unordered_set<char> valid_chars_set;
    for (const char &c : valid_chars) { valid_chars_set.insert(c); }
