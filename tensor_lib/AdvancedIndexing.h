...
public:
    ...
    inline SizeVector GetOutputShape() const {
        return output_shape_;
    }

    inline SizeVector GetIndexedShape() const {
        return indexed_shape_;
    }

    inline SizeVector GetIndexedStrides() const {
    ...
    static std::pair<Tensor, std::vector<Tensor>>
    ShuffleIndexedDimsToFront(const Tensor &tensor,
                              const std::vector<Tensor> &index_tensors);
    ...
    static Tensor RestrideTensor(const Tensor &tensor, int64_t dims_before,
                                 int64_t dims_indexed, SizeVector replacement_shape);
    ...
    static Tensor RestrideIndexTensor(const Tensor &index_tensor, int64_t dims_before,
                                      int64_t dims_after);
...
protected:
    ...
    ExpandBoolTensors(const std::vector<Tensor> &index_tensors);
...
public:
    ...
    AdvancedIndexer(const Tensor &src, const Tensor &dst,
                    const std::vector<Tensor> &index_tensors,
                    const SizeVector &indexed_shape, const SizeVector &indexed_strides,
                    AdvancedIndexerMode mode) : mode_(mode) {
        if (indexed_shape.size() != indexed_strides.size()) {
            utility::LogError("Internal error: indexed_shape's ndim {} does not equal to "
        ...
        for (const Tensor &index_tensor : index_tensors) {
            if (index_tensor.NumDims() != 0) { inputs.push_back(index_tensor); }
        }
        ...
        if (src.GetDtype() != dst.GetDtype()) {
            utility::LogError("src's dtype {} is not the same as dst's dtype {}.",
    ...
    inline OPEN3D_HOST_DEVICE int64_t GetIndexedOffset(int64_t workload_idx) const {
        ...
        for (int64_t i = 0; i < num_indices_; ++i) {
            int64_t index =
                *(reinterpret_cast<int64_t *>(indexer_.GetInputPtr(i + 1, workload_idx)));
            OPEN3D_ASSERT(index >= -indexed_shape_[i] && index < indexed_shape_[i]
                          && "Index out of bounds.");
