...
namespace open3d { namespace core { namespace shape_util {
...
static SizeVector ExpandFrontDims(const SizeVector &shape, int64_t ndims) {
    ...
    std::copy(shape.begin(), shape.end(), expanded_shape.begin() + ndims - shape.size());
...
bool IsCompatibleBroadcastShape(const SizeVector &l_shape, const SizeVector &r_shape) {
    ...
    if (l_ndims == 0 || r_ndims == 0) { return true; }
...
SizeVector BroadcastedShape(const SizeVector &l_shape, const SizeVector &r_shape) {
    if (!IsCompatibleBroadcastShape(l_shape, r_shape)) {
        utility::LogError("Shape {} and {} are not broadcast-compatible", l_shape,
                          r_shape);
    }
    ...
    for (int64_t i = 0; i < out_ndims; i++) {
        if (l_shape_filled[i] == 1) {
            ...
            utility::LogError("Internal error: dimension size {} is not compatible with "
...
SizeVector ReductionShape(const SizeVector &src_shape, const SizeVector &dims,
                          bool keepdim) {
    ...
    if (keepdim) {
        for (const int64_t &dim : dims) { out_shape[WrapDim(dim, src_ndims)] = 1; }
    } else {
...
int64_t WrapDim(int64_t dim, int64_t max_dim, bool inclusive) {
    if (max_dim <= 0) { utility::LogError("max_dim {} must be > 0.", max_dim); }
    int64_t min = -max_dim;
    int64_t max = inclusive ? max_dim : max_dim - 1;

    if (dim < min || dim > max) {
        utility::LogError("Index out-of-range: dim == {}, but it must satisfy {} <= dim "
                          "<= {}.",
                          dim, min, max);
    }
...
SizeVector InferShape(SizeVector shape, int64_t num_elements) {
    ...
    for (int64_t dim = 0, ndim = shape.size(); dim != ndim; dim++) {
        if (shape[dim] == -1) {
            if (has_inferred_dim) {
                utility::LogError("Proposed shape {}, but at most one dimension can be "
                                  "-1 (inferred).",
                                  shape.ToString());
            }
            inferred_dim = dim;
    ...
    if (num_elements == new_size
        ...
        if (has_inferred_dim) {
            ...
            // doesn't.
            if (new_size == 0) {
                utility::LogError("Cannot reshape tensor of 0 elements into shape {}, "
...
SizeVector Iota(int64_t n) {
    if (n < 0) { utility::LogError("Iota(n) requires n >= 0, but n == {}.", n); }
    SizeVector sv(n);
...
std::pair<bool, SizeVector> Restride(const SizeVector &old_shape,
    ...
    if (numel == 0) {
        for (int64_t view_d = new_shape.size() - 1; view_d >= 0; view_d--) {
            if (view_d == (int64_t)(new_shape.size() - 1)) {
                ...
                new_strides[view_d] =
                    std::max<int64_t>(new_shape[view_d + 1], 1) * new_strides[view_d + 1];
            }
        }
    ...
    if (view_d != -1) { return std::make_pair(false, SizeVector()); }
    return std::make_pair(true, new_strides);
}
